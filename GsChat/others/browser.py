...
class Browser:
    ...
    async def screenshot(
        ...
        *,
        ...
        wait_until: Literal[
            "domcontentloaded", "load", "networkidle", "load", "commit"
        ] = "networkidle",
        ...
        **kwargs,
        ...
        page = await browser.new_page(viewport=viewport_size, **kwargs)
        try:
            await page.goto(url, wait_until=wait_until, timeout=timeout)
            assert page
            if not elements:
                return await page.screenshot(
                    timeout=timeout, full_page=full_page
                )

            clip = None
            for e in elements:
                card = await page.wait_for_selector(
                    e, timeout=timeout, state="visible"
                )
                assert card
