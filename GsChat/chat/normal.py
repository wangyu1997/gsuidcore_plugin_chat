...
class NormalChat(BaseChat):
    def __init__(self, config=None):
        ...
        self.person_file: Path = (
            self.person_path / f"{self.config.person}.json"
        )
    ...
    async def _ask(self, user_id, bot: Bot, event: Event):
        ...
        try:
            ...
            if result is None:
                data = (
                    f"抱歉，{self.nickname}暂时不知道怎么回答你呢, 试试使用openai或者bing吧~"
                )
    ...
    async def init_data(self):
        if not os.path.exists(self.person_file):
            logger.warning(
                f"NormalChat 人设文件[{self.person_file.name}]不存在，已切换到默认配置"
            )
            self.person_file: Path = (
                self.person_path / f"{self.config.default}.json"
            )

        if not os.path.exists(self.person_file):
            logger.info("NormalChat 正在下载人设文件...")
            await download_file(self.person_file, self.download_url)

        if os.path.exists(self.person_file) and str(
            self.person_file
        ).endswith(".json"):
    ...
    async def normal_chat(self, msg, session=None, load_person=True):
        ...
        async with httpx.AsyncClient(
            ...
            message = message.lstrip(f"{self.nickname}：").lstrip(
                f"{self.nickname}:"
            )
            return message
