...
async def reserve_openai(bot: Bot, event: Event):
    ...
    await bot.send("openai会话已重置", at_sender=True)
...
async def openai_handle(bot: Bot, event: Event):
    ...
    if not apikey_allow:
        await bot.send("openai_api_key未设置, 无法访问", at_sender=True)
        return
    ...
    if uid not in openai_chat_dict:
        await openai_new_chat(bot, event)
        await bot.send("openai新会话已创建", at_sender=True)
    ...
    try:
        ...
        await bot.send(f'askError: {str(e)}多次askError请尝试发送"重置openai"', at_sender=True)
        return
...
async def openai_new_chat(bot: Bot, event: Event) -> None:
    ...
    openai_chat_dict[user_id] = {
        "chatbot": bot, "last_time": current_time, "sessions_number": 0, "isRunning": False}
