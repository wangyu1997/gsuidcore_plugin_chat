...
async def pretreatment(bot: Bot, event: Event):
    ...
    if uid not in bing_chat_dict:
        ...
        await bot.send("newbing新会话已创建", at_sender=True)
        return
...
async def bing_handle(bot: Bot, event: Event):
    ...
    try:
        ...
        await bot.send(f'askError: {str(e)}多次askError请尝试"重置bing"', at_sender=True)
        return
...
async def newbing_new_chat(bot: Bot, event: Event, style=config.newbing_style) -> None:
    ...
    bot = bingChatbot(cookies=random.choice(
        bing_cookies))  # 随机选择一个cookies创建一个Chatbot
    bing_chat_dict[user_id] = {
        "chatbot": bot, "last_time": current_time, "model": style, "isRunning": False}
