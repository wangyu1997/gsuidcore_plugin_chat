...
class ChatEngine:
  def __init__(self, config: CfgNode=None):
    self.name_map = {'bing': 'Bing','chat': 'Normal', 'openai': 'Openai'} # nickname->engine name
  ...
  def get_bot_info(self, event: Event):
    ...
    if not is_private:
      ...
      if group_id not in self.bot_user_map:
        self.bot_user_map[group_id] = {'group': False, 'engine': self.default_engine}
      group_chat = self.bot_user_map[group_id]['group']
      if group_chat:
        return group_id, True, self.bot_user_map[group_id]['engine']
    
    if is_private or (not group_chat):
      user_id = event.user_id
      if user_id not in self.bot_user_map:
        self.bot_user_map[user_id] = {'group': False, 'engine': self.default_engine}
