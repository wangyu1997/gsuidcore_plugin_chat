...
class NormalChat(BaseChat):
  ...
  async def _create(self, user_id):
    ...
    self.chat_dict[user_id] = {"session": [], "last_time": current_time,
  ...
  async def init_data(self):
    ...
    if os.path.exists(self.person_file) and str(self.person_file).endswith('.json'):
        ...
        try:
            per_data = json.loads(open(self.person_file, 'r').read())
            self.prompt.append(
                {'role': 'system', 'content': f'{per_data["system_prompt"].replace("_bot_name_", self.nickname)}'})
            for item in per_data['personality']:
                self.prompt.append(
                    {'role': 'user', 'content': f'{item["user"].replace("_bot_name_", self.nickname)}'})
                self.prompt.append(
                    {'role': 'assistant', 'content': f'{item["ai"].replace("_bot_name_", self.nickname)}'})
        except Exception as e:
